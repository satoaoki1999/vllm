...
def _check_early_stop_heuristic(
    ...
    generated. So, if the *best possible* score from any running beam is already worse than the *worst*
    finished beam, we can safely stop early.
...
def _get_top_k_continuations(
    ...
    output_log_probs = torch.full(
        (batch_size * num_beams, beams_to_keep),
        1e-9,
        dtype=torch.float32,
    ...
    if is_first_step:
        ...
        for idx in range(batch_size):
            ...
            for item_idx, (token_id, logprob_obj) in enumerate(lp_map.items()):
                ...
                output_token_ids[
                    idx, item_idx // beams_to_keep, item_idx % beams_to_keep
                ] = int(token_id)
                output_log_probs[
                    idx, item_idx // beams_to_keep, item_idx % beams_to_keep
