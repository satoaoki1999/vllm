...
def _get_token_prompts(tensor: torch.Tensor, pad_token_id: int):
    """
    ...
    first_nonpad = torch.where(any_nonpad, mask.float().argmax(dim=1),
                               torch.full((tensor.size(0),), tensor.size(1), device=tensor.device,
                                          dtype=torch.long)
                               )
    ...
    for i in range(tensor.size(0)):
        ...
        if start >= tensor.size(1):  # all-pads -> empty sequence
            processed = []
        else:
            processed = tensor[i, start:].tolist()
...
def _get_top_k_continuations(
    ...
    output_log_probs = torch.full(
        (batch_size * num_beams, beams_to_keep), 1e-9, dtype=torch.float32, device=running_sequences.device)
    output_token_ids = torch.zeros(
        (batch_size * num_beams, beams_to_keep), dtype=torch.long, device=running_sequences.device)

    if is_first_step:
        output_log_probs = _unflatten_beam_dim(
            output_log_probs, batch_size, num_beams)
        output_token_ids = _unflatten_beam_dim(
            output_token_ids, batch_size, num_beams)
        for idx in range(batch_size):
            ...
            for item_idx, (token_id, logprob_obj) in enumerate(lp_map.items()):
                ...
                output_token_ids[idx, item_idx // beams_to_keep,
                                   item_idx % beams_to_keep] = int(token_id)
                output_log_probs[idx, item_idx // beams_to_keep,
                                   item_idx % beams_to_keep] = float(logprob_obj.logprob)
    else:
    ...
    # flatten to [batch, num_beams * vocab_size]
    output_log_probs = torch.reshape(
        output_log_probs, (batch_size, num_beams * beams_to_keep))
    output_token_ids = torch.reshape(
        output_token_ids, (batch_size, num_beams * beams_to_keep))

    topk_log_probs, topk_indices = torch.topk(
        output_log_probs, k=beams_to_keep, dim=-1)
    ...
    topk_current_beam_indices = topk_indices // beams_to_keep
    topk_running_beam_indices = _gather_beams(
        running_beam_indices, topk_current_beam_indices)
    topk_running_sequences = _gather_beams(
...
def _get_running_beams_for_next_iteration(
    ...
    running_sequences = _gather_beams(
        topk_running_sequences, next_topk_indices)
    running_beam_scores = _gather_beams(
        topk_running_log_probs, next_topk_indices)
    running_beam_indices = _gather_beams(
...
def _update_finished_beams(
    ...
    beam_indices = _gather_beams(
        merged_beam_indices, topk_merged_indices)
    is_sent_finished = _gather_beams(
