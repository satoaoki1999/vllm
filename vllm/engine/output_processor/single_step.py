...
class SingleStepOutputProcessor(SequenceGroupOutputProcessor):
    ...
    def _process_sequence_group_outputs(self, seq_group: SequenceGroup,
                                        ...
                                        is_async: bool) -> None:
        def seq_group_add(seq_group: SequenceGroup, seq: Sequence):
            if seq.seq_id in seq_group.seqs_dict:
                raise ValueError(f"Sequence {seq.seq_id} already exists.")
            seq_group.seqs_dict[seq.seq_id] = seq
            seq_group.seqs.append(seq)

        def seq_group_remove(seq_group: SequenceGroup, seq_id: int):
            seq = seq_group.seqs_dict.pop(seq_id, None)
            if seq is None:
                return  # already deleted because of low score
                # raise ValueError(f"Sequence {seq_id} not found.")
            seq_group.seqs.remove(seq)
        ...
        for parent in parent_seqs:
            ...
            if len(child_samples) == 0:
                ...
                seq_group_remove(seq_group, parent.seq_id)
                for scheduler in self.scheduler:
        ...
        for seq, parent, is_new in all_finished_seqs[beam_width:]:
            if is_new:
                ...
                seq_group_remove(seq_group, seq.seq_id)
        ...
        if len(running_child_seqs) == 0:
            ...
            stop_beam_search = self._check_beam_search_early_stopping(
                early_stopping, sampling_params,
